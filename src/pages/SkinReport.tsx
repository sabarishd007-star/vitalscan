...
function MetricRow({ label, value, max = 10 }: { label: string; value: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-4 py-2 border-b border-gray-100 last:border-0">
      <span className="w-40 text-gray-600 text-sm">{label}</span>
...
export default function SkinReport() {
  ...
  const handleDownload = () => {
    ...
    pdf.text(`Skin Type: ${result.skinType}`, margin + 100, y);
    ...
    pdf.text("It does not diagnose diseases or replace professional medical advice.", margin, y + 5);
                ...
                {recs.morningRoutine.map((step) => (
                  <div key={step.order} className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-pink-600 text-xs">{step.product}</p>
                      <p className="text-gray-500 text-xs">{step.why}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3">
                {recs.nightRoutine.map((step) => (
                  <div key={step.order} className="flex gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800 text-sm">{step.step}</p>
                      <p className="text-violet-600 text-xs">{step.product}</p>
                      <p className="text-gray-500 text-xs">{step.why}</p>
                    </div>
                  </div>
                ))}
